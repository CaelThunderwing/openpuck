...
static bool g_x360RecoveryResetBoot = false;
...
void setup()
{
...
#endif
	loadCfg();
	faultDiagUsbBootTraceBegin(g_usbMode == MODE_XBOX360_CONSOLE);
	loadBonds();
	...
	// MODE_XBOX360_CONSOLE must present only the Xbox controller/security
	// interfaces. The stock 360 rejects the generic wake-mouse/WebUSB
	// interfaces that ordinary desktop XInput modes can tolerate.
	const bool xbox360Clean = (g_usbMode == MODE_XBOX360_CONSOLE);
	...
	if (dynamic) {
		...
		s_dynWantWakeMouse = !psClean && !xbox360Clean;
		s_dynWantWebusb = !psClean && !xbox360Clean;
		USBDevice.detach();
		delay(30);
		USBDevice.clearConfiguration();
		...
		if (!puckMode && !keepCdc && !psClean && !xbox360Clean)
			wakeHidBegin();
		...
		if (!puckMode && !psClean && !xbox360Clean)
			usb_web.begin();
...
// Xbox 360 USB-host lifecycle instrumentation.
// TinyUSB calls these public callbacks when the host configures, unconfigures,
// suspends or resumes the device. A USB BUS RESET is surfaced by TinyUSB as
// tud_umount_cb(), so these breadcrumbs let the existing flight recorder show
// what the Xbox host did during cold boot / warm reboot without a hardware sniffer.
//
// Trace detail values (all use FR_SAVE only as an existing generic recorder tag):
//   A1 = mount/configured
//   A2 = umount/unconfigured (also emitted on USB bus reset)
//   A3 = suspend
//   A4 = resume
//   A5 = recovery scheduled and consumed by loop()
//   A6 = synthetic detach
//   A7 = synthetic attach
//   A8 = legacy diagnostic: A2 arrived while uptime < 10 s (guard now removed)
//   A9 = first A2 accepted for pair detection
//   AA = second eligible A2 matched within the 250 ms window
//   AB = eligible A2 seen, but prior eligible A2 was outside the window
static volatile bool g_x360RecoveryScheduled = false;
static bool g_x360RecoveryDone = false;
static uint8_t g_x360RecoveryState = 0;
static uint32_t g_x360RecoveryDeadlineMs = 0;
static volatile uint32_t g_x360LastUmountMs = 0;
static volatile uint8_t g_x360LateUmountCount = 0;

#ifndef X360_AUTORECOVERY_ENABLE
#define X360_AUTORECOVERY_ENABLE 0
#endif

#define X360_RECOVERY_A2_WINDOW_MS        250u
#define X360_RECOVERY_AFTER_A2_MS        1000u
#define X360_RECOVERY_DETACH_MS           300u

static void xbox360ColdBootRecoveryTask(void)
{
        if (g_usbMode != MODE_XBOX360_CONSOLE)
                return;

        // g_x360RecoveryDone prevents a SECOND recovery cycle, but state 2
        // still has to run so the USB device can re-attach after the synthetic
        // detach. The previous guard returned here immediately after A6 and
        // stranded the Puck detached forever.
        if (g_x360RecoveryDone && g_x360RecoveryState == 0)
                return;

        uint32_t now = millis();

        if (g_x360RecoveryScheduled && g_x360RecoveryState == 0) {
                g_x360RecoveryScheduled = false;
                g_x360RecoveryState = 1;
                // The Xbox's cold-boot signature observed repeatedly is two
                // umount/bus-reset callbacks about 30 ms apart. Give the host
                // another second to finish bringing its USB stack up, then
                // synthesize the known-good hotplug.
                g_x360RecoveryDeadlineMs = now + X360_RECOVERY_AFTER_A2_MS;
                faultDiagUsbBootTrace(0xA5);
                faultDiagTrace(FR_SAVE, 0xA5);
        }

        if (g_x360RecoveryState == 1 &&
            (int32_t)(now - g_x360RecoveryDeadlineMs) >= 0) {
                g_x360RecoveryDone = true;
                g_x360RecoveryState = 2;
                USBDevice.detach();
                faultDiagUsbBootTrace(0xA6);
                faultDiagTrace(FR_SAVE, 0xA6);
                g_x360RecoveryDeadlineMs = now + X360_RECOVERY_DETACH_MS;
                return;
        }

        if (g_x360RecoveryState == 2 &&
            (int32_t)(now - g_x360RecoveryDeadlineMs) >= 0) {
                // Soft attach did not reproduce a physical USB-C reconnect.
                // Stay detached and reboot the MCU so USB returns through the
                // complete normal initialization path.
                faultDiagUsbBootTrace(0xA7); // recovery reset issued
                faultDiagTrace(FR_SAVE, 0xA7);
                faultDiagArmXbox360RecoveryReset();
                NVIC_SystemReset();
                while (1) {}
        }
}
...
{
        if (g_usbMode == MODE_XBOX360_CONSOLE) {
                faultDiagUsbBootTrace(0xA1);
                faultDiagTrace(FR_SAVE, 0xA1);
        }
}
...
{
        if (g_usbMode == MODE_XBOX360_CONSOLE) {
                faultDiagUsbBootTrace(0xA2);
                faultDiagTrace(FR_SAVE, 0xA2);

                // Cold-boot signature seen on the 360: two A2 callbacks roughly
                // 30 ms apart. The previous uptime guard was disproven by the
                // A8 diagnostics, so the pair itself is the recovery trigger.
                // Only arm here; loop() performs the detach/attach.
                //
                // Diagnostic markers below record the ACTUAL branch decisions
                // made here, so we no longer have to infer detector behavior
                // from the later reconstructed st255 A2 records.
                uint32_t now = millis();

                if (g_x360RecoveryDone || g_x360RecoveryResetBoot)
                        return;

                // The detector markers proved the Xbox cold-boot A2 pair is
                // arriving while millis() is still below 10 s, even when the
                // Puck was deliberately left connected beforehand. Do not gate
                // recovery on MCU uptime; the repeatable double-A2 signature
                // itself is now the guard.
                uint32_t dt = now - g_x360LastUmountMs;

                if (!g_x360LateUmountCount) {
                        g_x360LateUmountCount = 1;
                        g_x360LastUmountMs = now;
                        faultDiagUsbBootTrace(0xA9);
                        return;
                }

                if (dt <= X360_RECOVERY_A2_WINDOW_MS) {
                        faultDiagUsbBootTrace(0xAA);
#if X360_AUTORECOVERY_ENABLE
                        g_x360RecoveryScheduled = true;
#endif
                        g_x360LateUmountCount = 0;
                        g_x360LastUmountMs = now;
                        return;
                }

                // We had a previous eligible A2, but it was too old to form
                // the cold-boot pair. Treat this A2 as the first of a new pair.
                faultDiagUsbBootTrace(0xAB);
                g_x360LateUmountCount = 1;
                g_x360LastUmountMs = now;
        }
}
...
extern "C" void tud_suspend_cb(bool remote_wakeup_en)
{
        ...
        if (g_usbMode == MODE_XBOX360_CONSOLE) {
                faultDiagUsbBootTrace(0xA3);
                faultDiagTrace(FR_SAVE, 0xA3);
        }
}
...
{
        if (g_usbMode == MODE_XBOX360_CONSOLE) {
                faultDiagUsbBootTrace(0xA4);
                faultDiagTrace(FR_SAVE, 0xA4);
        }
}
...
void loop()
{
    ...
    faultDiagUsbBootTraceTask(); // persist Mode-11 USB trace after 10 s of control-bus quiet
    xbox360ColdBootRecoveryTask(); // one-shot late-mount synthetic hotplug; loop-context only
