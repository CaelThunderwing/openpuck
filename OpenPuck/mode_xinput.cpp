...
// Stock Xbox 360 device-level identity for MODE_XBOX360_CONSOLE only.
static tusb_desc_device_t const g_x360DeviceDesc __attribute__((aligned(4))) = {
...
extern "C" uint8_t const *__wrap_tud_descriptor_device_cb(void)
{
    if (g_xbox360ConsoleMode)
        return reinterpret_cast<uint8_t const *>(&g_x360DeviceDesc);
    return __real_tud_descriptor_device_cb();
}


// Full OEM XSM3 interface string descriptor (178 bytes).
// Mode 11 bypasses the short generic TinyUSB string scratch buffer for index 4.
static const uint16_t g_xsm3FullStringDesc[] __attribute__((aligned(4))) = {
    0x03B2, 0x0058, 0x0062, 0x006F, 0x0078, 0x0020, 0x0053, 0x0065,
    0x0063, 0x0075, 0x0072, 0x0069, 0x0074, 0x0079, 0x0020, 0x004D,
    0x0065, 0x0074, 0x0068, 0x006F, 0x0064, 0x0020, 0x0033, 0x002C,
    0x0020, 0x0056, 0x0065, 0x0072, 0x0073, 0x0069, 0x006F, 0x006E,
    0x0020, 0x0031, 0x002E, 0x0030, 0x0030, 0x002C, 0x0020, 0x00A9,
    0x0020, 0x0032, 0x0030, 0x0030, 0x0035, 0x0020, 0x004D, 0x0069,
    0x0063, 0x0072, 0x006F, 0x0073, 0x006F, 0x0066, 0x0074, 0x0020,
    0x0043, 0x006F, 0x0072, 0x0070, 0x006F, 0x0072, 0x0061, 0x0074,
    0x0069, 0x006F, 0x006E, 0x002E, 0x0020, 0x0041, 0x006C, 0x006C,
    0x0020, 0x0072, 0x0069, 0x0067, 0x0068, 0x0074, 0x0073, 0x0020,
    0x0072, 0x0065, 0x0073, 0x0065, 0x0072, 0x0076, 0x0065, 0x0064,
    0x002E,
};

extern "C" uint16_t const *
__real_tud_descriptor_string_cb(uint8_t index, uint16_t langid);

extern "C" uint16_t const *
__wrap_tud_descriptor_string_cb(uint8_t index, uint16_t langid)
{
    if (g_xbox360ConsoleMode && index == 4)
        return g_xsm3FullStringDesc;
...
#define XINPUT_AUX1_DESC_LEN (9 + 27 + 7 + 7 + 7 + 7)
#define XINPUT_AUX2_DESC_LEN (9 + 9 + 7)
...
static XiAuxOut g_xiAuxOut[2];
static uint8_t g_xiAuxOutCount = 0;
...
static void xi_reset(uint8_t rhport)
{
    ...
    g_xiAuxOutCount = 0;
    memset(g_xiAuxOut, 0, sizeof g_xiAuxOut);
			...
			uint16_t max_len)
{
    ...
    // Licensed Xbox 360 auxiliary interfaces. Claim their endpoints so the
    // stock console sees a complete four-interface wired-controller function.
    if (g_xbox360ConsoleMode && itf->bInterfaceClass == 0xFF &&
        itf->bInterfaceSubClass == 0x5D &&
        (itf->bInterfaceProtocol == 0x03 || itf->bInterfaceProtocol == 0x02)) {
            uint8_t const *p = (uint8_t const *)itf + itf->bLength;
            uint8_t const *end = (uint8_t const *)itf + max_len;
            uint16_t used = itf->bLength;
            uint8_t opened = 0;

            while (p + 2 <= end && opened < itf->bNumEndpoints) {
                    ...
                    if (btype == TUSB_DESC_ENDPOINT) {
                            ...
                            if (tu_edpt_dir(ep->bEndpointAddress) == TUSB_DIR_OUT &&
                                g_xiAuxOutCount < 2) {
                                    XiAuxOut &A = g_xiAuxOut[g_xiAuxOutCount++];
                                    A.ep = ep->bEndpointAddress;
                                    usbd_edpt_xfer(rhport, A.ep, A.buf, sizeof A.buf);
                            }
                            opened++;
                    }
                    used += blen;
            ...
            return opened == itf->bNumEndpoints ? used : 0;
    }
...
// XSM3 request breadcrumbs for the existing flight recorder.
// Logged only at SETUP so each control transaction produces one entry:
// B1=0x81 identification, B2=0x82 challenge-init, B3=0x83 response,
// B4=0x84 ack, B6=0x86 state, B7=0x87 verify.
static inline void xiTraceXsm3Setup(uint8_t stage, uint8_t request)
{
        if (stage != CONTROL_STAGE_SETUP)
                return;
        uint8_t detail;
        switch (request) {
        case 0x81: detail = 0xB1; break;
        case 0x82: detail = 0xB2; break;
        case 0x83: detail = 0xB3; break;
        case 0x84: detail = 0xB4; break;
        case 0x86: detail = 0xB6; break;
        case 0x87: detail = 0xB7; break;
        default: return;
        }
        faultDiagUsbBootTrace(detail);
        faultDiagTrace(FR_SAVE, detail);
}
		    ...
		    tusb_control_request_t const *req)
{
	...
	xiTraceXsm3Setup(stage, req->bRequest);
...
bool xbox360ConsoleXsm3ControlXfer(uint8_t rhport, uint8_t stage,
                                   tusb_control_request_t const *req)
{
        return xi_ctrl(rhport, stage, req);
}
...
static bool xi_xfer(uint8_t rhport, uint8_t ep, xfer_result_t res, uint32_t n)
{
    ...
    for (uint8_t i = 0; i < g_xiAuxOutCount; i++) {
            if (ep == g_xiAuxOut[i].ep) {
                    (void)n;
                    usbd_edpt_xfer(rhport, ep, g_xiAuxOut[i].buf,
                                   sizeof g_xiAuxOut[i].buf);
                    return true;
            }
    }
	for (int s = 0; s < NSLOT; s++) {
...
class Adafruit_USBD_XInput : public Adafruit_USBD_Interface {
					...
					uint16_t bufsize) override
	{
		...
		const uint8_t t[XINPUT_DESC_LEN] = {
			...
			// Xbox 360 XInput gamepad descriptor.
			// Match Santroller's known-working descriptor byte-for-byte for subtype 0x01.
			0x11, 0x21, 0x10, 0x01, 0x01, 0x25, epin, 0x14,
			0x03, 0x03, 0x03, 0x04, 0x13, (g_xbox360ConsoleMode ? 0x02 : epout), 0x08, 0x03, 0x03,

			// Endpoint IN: 32-byte interrupt, 1 ms interval.
			7, TUSB_DESC_ENDPOINT, epin, TUSB_XFER_INTERRUPT,
			U16_TO_U8S_LE(0x20), (g_xbox360ConsoleMode ? 4 : 1),

			// Endpoint OUT: match Santroller's 1 ms interval.
			7, TUSB_DESC_ENDPOINT, (g_xbox360ConsoleMode ? 0x02 : epout), TUSB_XFER_INTERRUPT, U16_TO_U8S_LE(0x20), (g_xbox360ConsoleMode ? 8 : 1)
		};
...
class Adafruit_USBD_XInputAux1 : public Adafruit_USBD_Interface {
    public:
    uint16_t getInterfaceDescriptor(uint8_t, uint8_t *buf,
                                    uint16_t bufsize) override
    {
            if (!buf)
                    return XINPUT_AUX1_DESC_LEN;
            if (bufsize < XINPUT_AUX1_DESC_LEN)
                    return 0;

            uint8_t itfnum = TinyUSBDevice.allocInterface(1);
            uint8_t epin1 = TinyUSBDevice.allocEndpoint(TUSB_DIR_IN);
            uint8_t epout1 = TinyUSBDevice.allocEndpoint(TUSB_DIR_OUT);
            uint8_t epin2 = TinyUSBDevice.allocEndpoint(TUSB_DIR_IN);
            uint8_t epout2 = TinyUSBDevice.allocEndpoint(TUSB_DIR_OUT);

            const uint8_t t[XINPUT_AUX1_DESC_LEN] = {
                    9, TUSB_DESC_INTERFACE, itfnum, 0x00, 0x04,
                    0xFF, 0x5D, 0x03, 0x00,

                    0x1B, 0x21, 0x10, 0x01, 0x01, 0x01,
                    0x83, 0x40, 0x01, 0x04, 0x20, 0x16,
                    0x85, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x16,
                    0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,

                    7, TUSB_DESC_ENDPOINT, 0x83, TUSB_XFER_INTERRUPT,
                    U16_TO_U8S_LE(0x20), 2,
                    7, TUSB_DESC_ENDPOINT, 0x04, TUSB_XFER_INTERRUPT,
                    U16_TO_U8S_LE(0x20), 4,
                    7, TUSB_DESC_ENDPOINT, 0x85, TUSB_XFER_INTERRUPT,
                    U16_TO_U8S_LE(0x20), 64,
                    7, TUSB_DESC_ENDPOINT, 0x05, TUSB_XFER_INTERRUPT,
                    U16_TO_U8S_LE(0x20), 16
            };
...
class Adafruit_USBD_XInputAux2 : public Adafruit_USBD_Interface {
    public:
    uint16_t getInterfaceDescriptor(uint8_t, uint8_t *buf,
                                    uint16_t bufsize) override
    {
            if (!buf)
                    return XINPUT_AUX2_DESC_LEN;
            if (bufsize < XINPUT_AUX2_DESC_LEN)
                    return 0;

            uint8_t itfnum = TinyUSBDevice.allocInterface(1);
            uint8_t epin = TinyUSBDevice.allocEndpoint(TUSB_DIR_IN);

            const uint8_t t[XINPUT_AUX2_DESC_LEN] = {
                    9, TUSB_DESC_INTERFACE, itfnum, 0x00, 0x01,
                    0xFF, 0x5D, 0x02, 0x00,

                    0x09, 0x21, 0x10, 0x01, 0x01, 0x22, 0x86, 0x07, 0x00,

                    7, TUSB_DESC_ENDPOINT, 0x86, TUSB_XFER_INTERRUPT,
                    U16_TO_U8S_LE(0x20), 16
            };
            memcpy(buf, t, sizeof t);
            return sizeof t;
...
static Adafruit_USBD_XInputAux1 g_xinputAux1;
static Adafruit_USBD_XInputAux2 g_xinputAux2;
...
{
    return g_xbox360ConsoleMode ? 1u : (uint8_t)NSLOT;
}
void XboxController::usbIdentity()
{
	...
	if (g_xbox360ConsoleMode)
        USBDevice.setManufacturerDescriptor("©Microsoft Corporation");
    else
...
void XboxController::beginPool()
{
    ...
    // Console-mode XSM3 string registration is deferred to mountSlots(),
    // after usbReenumerate() has called USBDevice.clearConfiguration().
...
void XboxController::mountSlots(uint8_t k)
{
    ...
    if (g_xbox360ConsoleMode) {
            ...
            // Licensed wired-controller topology:
            // IF0 FF/5D/01 gamepad
            // IF1 FF/5D/03 auxiliary
            // IF2 FF/5D/02 auxiliary
            // IF3 FF/FD/13 XSM3 security
            USBDevice.addInterface(g_xinput[0]);
            USBDevice.addInterface(g_xinputAux1);
            USBDevice.addInterface(g_xinputAux2);

            // clearConfiguration() resets the string pool on every dynamic
            // rebuild, so XSM3 string registration must happen here.
