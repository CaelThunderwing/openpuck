...
__wrap_tud_vendor_control_xfer_cb(uint8_t rhport, uint8_t stage,
				  const tusb_control_request_t *request)
{
        // TinyUSB 1.7.0 sends all VENDOR requests here before recipient routing.
        // Forward only Xbox Security Method 3 interface-3 requests.
        if (g_usbMode == MODE_XBOX360_CONSOLE &&
            (request->bmRequestType & 0x60) == 0x40 &&
            (request->bmRequestType & 0x1F) == 0x01 &&
	...
	while (usb_web.available()) {
		...
		for (;;) {
			if (n == 0)
				break;
			uint8_t op = buf[0];
			// Includes the upstream 0x16 test-rumble command and this fork's later diagnostics/update opcodes.
			if ((op < 0x01 || op > 0x1B) &&
